  ...
  return (
    ...
    >
        ...
        <div className="relative group">
          ...
          <div className="absolute top-full left-1/2 -translate-x-1/2 bg-black/70 border border-[#abdc57] rounded-lg shadow-lg w-40 z-50 hidden group-hover:block">
            <div className="py-1">
                ...
                <span className="block px-4 py-2 text-sm text-white/50 cursor-not-allowed">3. Soon</span>
                <span className="block px-4 py-2 text-sm text-white/50 cursor-not-allowed">4. Soon</span>
                <span className="block px-4 py-2 text-sm text-white/50 cursor-not-allowed">5. Soon</span>
                <span className="block px-4 py-2 text-sm text-white/50 cursor-not-allowed">6. Soon</span>
                <span className="block px-4 py-2 text-sm text-white/50 cursor-not-allowed">7. Soon</span>
            </div>
          </div>
        </div>
      ...
      <div className="hidden md:flex items-center justify-end space-x-2">
        <Link to="/login">
          <button className="text-xs px-3 py-1 border border-[#AEE67F] font-bold text-[#AEE67F] rounded-lg hover:bg-[#42582b]">Log in</button>
        </Link>
        <Link to="/signup">
          <button className="text-xs px-3 py-1 bg-[#AEE67F] font-semibold text-black rounded-lg hover:opacity-90">Sign up</button>
        </Link>
      </div>

      <button className="md:hidden absolute right-4 top-3 text-white" onClick={toggleMenu}>
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-70 z-40 md:hidden">
          ...
          <div className="flex flex-col items-center justify-center h-full space-y-6">
            <Link to="/" onClick={toggleMenu} className="text-white text-base">Home</Link>
            <Link to="/about" onClick={toggleMenu} className="text-white text-base">About</Link>
            <Link to="/program/bmc" onClick={toggleMenu} className="text-white text-base">Program</Link>
            <a href="https://linktr.ee/mnf.summit25" onClick={toggleMenu} className="text-white text-base">Merchandise</a>
            <div className="flex space-x-4 mt-4">
              <Link to="/login" onClick={toggleMenu}>
                <button className="text-xs px-4 py-1 bg-[#2e3e15] text-white rounded-lg hover:bg-[#42582b]">Log in</button>
              </Link>
              <Link to="/signup" onClick={toggleMenu}>
                <button className="text-xs px-4 py-1 bg-[#abdc57] text-black rounded-lg hover:opacity-90">Sign up</button>
              </Link>
            </div>
          </div>
        </div>

...
import { CardInfoSection } from "@/components/aboutPage/CardInfoSection";
import { ChallengesSection } from "@/components/aboutPage/ChallengesSection";
import { ComponentHeaderSection } from "@/components/aboutPage/ComponentHeaderSection";
import { ComponentNodeSection } from "@/components/aboutPage/ComponentNodeSection";
...
import { MissionDetailsSection } from "@/components/aboutPage/MissionDetailsSection";
import { MissionSection } from "@/components/aboutPage/MissionSection";
import { OurMissionsSection } from "@/components/aboutPage/OurMissionsSection";
import { ParticipantSection } from "@/components/aboutPage/ParticipantSection";
      ...
      <div className="bg-black overflow-hidden w-[1440px] h-[10240px] relative">
        <div className="absolute w-[1725px] h-[8697px] top-[1020px] left-[-158px]">
          ...
          {ambassadorImages.map((ambassador, index) => (
            <img
              ...
              className={`absolute w-[${
                index === 3 ? "227px" : index === 0 ? "302px" : "303px"
              }] h-[302px] top-[7992px] left-[${
                287 + index * 361
              }px] object-cover`}
              alt={ambassador.alt}
              src={ambassador.src}
          ...
          {focusItems.map((item, index) => (
            <div
              key={item.id}
              className={`absolute w-[98px] h-[98px] top-[4546px] left-[${
                209 + index * (index === 1 ? 493 : 462)
              }px] bg-[#b9fd50] rounded-[49px]`}
            />
          ))}
          ...
          {milestoneItems.map((milestone, index) => (
            <React.Fragment key={milestone.id}>
              <div
                className={`absolute w-[163px] top-[${
                  5442 + (index === 1 ? 1 : index === 2 ? -1 : 0)
                }px] left-[${
                  424 + index * 423
                }px] [font-family:'Rubik-Medium',Helvetica] font-medium text-white text-5xl tracking-[-0.96px] leading-[62px]`}
              >
                {milestone.count}
              </div>
              <div
                className={`absolute top-[${
                  5510 + (index === 1 ? 1 : index === 2 ? -1 : 0)
                }px] left-[${
                  424 + index * 423
                }px] [font-family:'Plus_Jakarta_Sans-SemiBold',Helvetica] font-semibold text-white text-2xl tracking-[-0.24px] leading-[normal]`}
              >
                {milestone.title}
              </div>
              <img
                className={`absolute w-[100px] h-[100px] top-[${
                  5442 + (index === 2 ? -2 : 0)
                }px] left-[${301 + index * 422}px]`}
                alt="Milestone icon"
                src={milestone.icon}
              />
            </React.Fragment>
            ...
            <React.Fragment key={item.id}>
              <p
                className={`absolute w-[${
                  ...
                  242 + index * (index === 1 ? 461 : 462)
                }px] [font-family:'Plus_Jakarta_Sans-Bold',Helvetica] font-bold text-[#ffffffcc] text-lg tracking-[0] leading-[25.2px]`}
              >
                {item.description}
              </p>
              <div
                className={`absolute w-[${
                  index === 0 ? "400px" : index === 1 ? "313px" : "352px"
                }] top-[4686px] left-[${
                  242 + index * (index === 1 ? 461 : 462)
                }px] [font-family:'Rubik-Medium',Helvetica] font-medium text-white text-[26px] tracking-[-0.26px] leading-[normal]`}
              >
                {item.title}
              </div>
              {item.vector && (
                <img
                  className={`absolute w-[${
                    index === 0 ? "348px" : "315px"
                  }] h-[15px] top-[${4589 + (index === 1 ? -1 : 0)}px] left-[${
                    327 + index * 489
                  }px]`}
                  alt="Vector"
                  src={item.vector}
        ...
        <header className="absolute w-[1520px] h-[1020px] top-0 -left-10">
          ...
          >
                    ...
                    {item.hasDropdown ? (
                      <div className="relative w-[78.4px] h-3.5">
                        ...
                        <div
                          ...
                          style={{
                            backgroundImage: `url(/images/vector-4-stroke.svg)`,
                          }}
                        />
                      </div>
                    ) : (
                      <a
                        href={item.href}
                        className={`w-fit mt-[-0.77px] [font-family:'Inter-Bold',Helvetica] text-white text-[13.9px] leading-[13.9px] whitespace-nowrap ${
                          index === 0 ? "relative" : "relative"
                        } font-bold tracking-[0]`}
                      >
                        {item.label}
                      </a>
                    )}
            ...
            <div
              ...
              style={{
                backgroundImage: `url(./images/logo-sxc-putih-2-2.png)`,
              }}
            />
          </nav>
        </header>

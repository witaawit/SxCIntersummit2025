...
const { Institution } = require("@prisma/client");
...
exports.register = async (req, res) => {
  ...
  const user = await createTempUser({
    ...
    otp: OTP,
    purpose: "register",
  }); // insert temp user dengan field name, email, password, dan otp
...
exports.forgotPassword = async (req, res) => {
  ...
  res
    .status(200)
    .json({ message: "OTP untuk reset password telah dikirim ke email Anda" });
};
...
exports.verifyOTP = async (req, res) => {
  ...
  if (tempUser.purpose === "register") {
    ...
    if (tempUser.referral) {
      referralData = await findReferralByCode(tempUser.referral);
      if (tempUser.referral && !referralData) {
        return res.status(400).json({ message: MSG.INVALID_REFERRAL });
      }
    }
    const referralId = referralData ? referralData.id : null;
  ...
  return res
    .status(400)
    .json({ message: "Invalid purpose for OTP verification" });
};
